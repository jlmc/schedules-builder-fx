...
public abstract class MachineStatesController extends ContentController {
	 ...
	 *            the resources
	 */
	private void loadsubviews(final Map<State, String> subviews,
	 ...
	 */
	private ContentController loadSubController(final State state,
			final String subController, final ResourceBundle resources) {
		try {
			final StateController controller = loadSubView(subController,
					resources);
	...
	protected <T> T loadSubView(final String subController,
		...
		try {
			final Result loadResult = this.fxmlLoader.load(
					MachineStatesController.class.getResource(subController),
					resources);
